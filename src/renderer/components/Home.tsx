...
export default class Home extends React.Component<{}, AppState> {
  ...
  private loadLogins = () => {
    this.setState({ refreshing: true }, () => {
      ...
      childProcess.exec(cmd, (error, stdout, stderr) => {
        ...
        let logins: Login[] = [];
        if (stdout) {
          const results = JSON.parse(stdout) as OrgListResult
  ...
  private launchAuth = () => {
    this.setState({ waitingForAuth: true }, () => {
      ...
      if (this.state.instanceUrl) {
        const instanceUrl = this.state.instanceUrl.startsWith('https://') ? this.state.instanceUrl : 'https://' + this.state.instanceUrl;
        params += ' -r ' + instanceUrl
      }

      const cmd = "sfdx force:auth:web:login " + params
  ...
  private renderBody = () => {
    const logins = [...this.state.logins].filter(ln => {
